...
void traxPlancton(OceanCell ocean[X_SIZE][Y_SIZE], int y, int x) {
		...
		int random = rand() % 2;
		if (random == 0) { 
			if ((x - 1 >= 0) && (ocean[y][x - 1].alive == 0))
			{
				bornPlankton(ocean, x - 1, y);
			}
			else if ((x + 1 < X_SIZE) && (ocean[y][x + 1].alive == 0)) {
				bornPlankton(ocean, x + 1, y);
			}
			else if ((y - 1 >= 0) && (ocean[y - 1][x].alive == 0)) {
				bornPlankton(ocean, x, y - 1);
			}
			else if ((y + 1 < Y_SIZE) && (ocean[y + 1][x].alive == 0)) {
				bornPlankton(ocean, x, y + 1);
			}
		}
	return;
